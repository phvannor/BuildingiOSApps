...
-(QSTodoService *)init
{
    ...
    if (self)
    {
        ...
        MSClient *client = [MSClient clientWithApplicationURLString:@"https://philsdemobackup.azure-mobile.net/"
                                                     applicationKey:@"zSRiibORahtLKUxauGpXmWsuqeEZot37"];
...
- (void) addAPNSToken:(NSString*) token
    ...
    MSTable *pushInfoTable = [_client tableWithName:@"PushInfo"];
    NSDictionary *item = @{ @"token" : token };
    
    [pushInfoTable insert:item completion:^(NSDictionary *result, NSError *error) {
